...
class SecurityAuditLog:
    ...
    def append_in_connection(
        self, connection: sqlite3.Connection, event_type: str, *, success: bool,
        actor_user_id: int | None = None, target_user_id: int | None = None,
        request_id: str | None = None, metadata: dict[str, str] | None = None,
    ) -> SecurityEvent:
        event = SecurityEvent(
            event_id=str(uuid.uuid4()), created_at=datetime.now(timezone.utc).isoformat(),
            event_type=event_type, success=success, actor_user_id=actor_user_id,
            target_user_id=target_user_id, request_id=request_id, metadata=metadata,
        )
        ...
        connection.execute(
            ...
            (event.event_id,event.created_at,event.event_type,event.actor_user_id,
             event.target_user_id,int(event.success),event.request_id,payload,previous,entry_hash),
        )
        return event
