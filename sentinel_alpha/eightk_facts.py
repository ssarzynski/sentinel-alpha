...
# These rules require explicit disclosure language in document content. Item
# numbers alone are never directional evidence.
_ADVERSE_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    (
        "4.02",
        re.compile(
            r"\b(?:should\s+no\s+longer\s+be\s+relied\s+upon|"
            r"should\s+not\s+be\s+relied\s+upon|non[- ]reliance\s+on\s+previously\s+issued\s+financial\s+statements)\b",
            re.IGNORECASE,
        ),
    ),
    (
        "3.01",
        re.compile(
            r"\b(?:notice\s+of\s+delisting|failure\s+to\s+satisfy\s+(?:a\s+)?continued\s+listing\s+rule|"
            r"noncompliance\s+with\s+(?:a\s+)?continued\s+listing\s+standard)\b",
            re.IGNORECASE,
        ),
    ),
)
...
def parse_explicit_adverse_8k_facts(document: str) -> tuple[EightKEvent, ...]:
    ...
    ambiguous, or item-number-only text returns no adverse facts and therefore
