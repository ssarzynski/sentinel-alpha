...
FRED_SERIES_POLICIES = {
    "DGS2": MacroSourcePolicy("Federal Reserve Bank of St. Louis", "fred", "federal_reserve_board"),
    "DGS10": MacroSourcePolicy("Federal Reserve Bank of St. Louis", "fred", "federal_reserve_board"),
    "FEDFUNDS": MacroSourcePolicy("Federal Reserve Bank of St. Louis", "fred", "federal_reserve_board"),
...
TREASURY_POLICY = MacroSourcePolicy("U.S. Department of the Treasury", "daily_treasury_yield_curve", "us_treasury")
def source_identity_for_macro_input(item: MacroInput) -> SourceIdentity:
    ...
    return SourceIdentity(source_id=source, provider=policy.provider, channel=policy.channel, independent_group=policy.independent_group)
def normalize_macro_input(item: MacroInput) -> NormalizedRecord:
    source = source_identity_for_macro_input(item)
    return normalize_record(asset=MACRO_ASSET, metric=item.metric, value=item.value, source=source, observed_at=item.observed_at, statement=f"{item.metric}={item.value}", reference=item.source, quality="authoritative")
def normalize_macro_inputs(inputs: Mapping[str, MacroInput]) -> list[NormalizedRecord]:
    ...
    for metric, item in inputs.items():
        if metric != item.metric:
            raise ValueError(f"macro metric key {metric!r} does not match input metric {item.metric!r}")
