...
class TreasuryClient:
    ...
    def latest(self, *, year: int | None = None) -> TreasuryYieldObservation:
        ...
        for entry in root.findall("{http://www.w3.org/2005/Atom}entry"):
            props = entry.find(properties_tag)
            if props is None:
                continue
...
def treasury_macro_inputs(
    observation: TreasuryYieldObservation, *, max_age
) -> dict[str, MacroInput]:
    return {
        "treasury_2y": MacroInput(
            metric="treasury_2y",
            value=observation.treasury_2y,
        ...
        "treasury_10y": MacroInput(
            metric="treasury_10y",
            value=observation.treasury_10y,
            observed_at=observation.observed_at,
            source=TREASURY_SOURCE,
            max_age=max_age,
        ),
...
def cross_validate_yields(
    primary: Mapping[str, MacroInput],
    comparison: Mapping[str, MacroInput],
    *,
