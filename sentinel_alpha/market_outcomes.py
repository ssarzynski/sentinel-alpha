...
def label_forward_outcomes(
    ...
    *,
    ...
    normalized_anchor = as_utc(anchor_at)
    anchor_index = next(
        (index for index, row in enumerate(rows) if as_utc(row.observed_at) == normalized_anchor),
        None,
    )
    ...
    for horizon in horizons:
        ...
        if target_index >= len(rows):
            results.append(ForwardOutcome(symbol.strip().upper(), anchor.observed_at, float(anchor.price), horizon, None, None, None))
            continue
        target = rows[target_index]
        forward_return = None if anchor.price == 0 else float(target.price) / float(anchor.price) - 1.0
        results.append(ForwardOutcome(symbol.strip().upper(), anchor.observed_at, float(anchor.price), horizon, target.observed_at, float(target.price), forward_return))
    return results
