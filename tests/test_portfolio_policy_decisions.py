...
def save_history(db):
    for index in range(22):
        persist_price_observation(db, PriceObservationInput("NVDA", T0 - timedelta(days=21-index), 100 + index + index % 2, "USD", "A", "exchange", f"n-{index}"))


def test_evaluation_uses_persisted_policy_and_records_snapshot():
    db = db_session()
    portfolio = setup(db, {"max_position_weight": 0.5, "max_annualized_volatility": 10.0})
    save_history(db)
    decision = evaluate_and_record_portfolio_policy(db, portfolio, min_observations=20)
    ...
    assert decision.risk_json["observations"] == 21
    assert any(item["code"] == "max_position_weight" for item in decision.findings_json)
    assert decision.human_review_status == "pending"


def test_unavailable_risk_is_preserved_in_ledger_not_hidden():
    db = db_session()
    portfolio = setup(db, {"max_annualized_volatility": 0.2})
    decision = evaluate_and_record_portfolio_policy(db, portfolio)
    assert decision.risk_json is None
    assert decision.risk_data_status.startswith("unavailable:")
    assert any(item["code"] == "risk_unavailable" for item in decision.findings_json)


def test_history_is_append_only_across_repeated_evaluations():
    db = db_session()
    portfolio = setup(db, {})
    first = evaluate_and_record_portfolio_policy(db, portfolio)
    second = evaluate_and_record_portfolio_policy(db, portfolio)
