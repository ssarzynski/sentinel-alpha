...
def test_last_active_admin_cannot_be_disabled(tmp_path):
    ...
    with pytest.raises(ValueError,match="administrator cannot deactivate own active session account"):
...
def test_last_active_admin_cannot_be_demoted_by_another_admin_state(tmp_path):
    ...
    with pytest.raises(ValueError,match="administrator cannot remove own administrator role"):
